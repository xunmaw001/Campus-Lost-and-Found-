		...
		<form class="app-update-pv">
			<view
				...
				<textarea
					...
					v-model="ruleForm.xiaoyuanshiwuzhaolingLiuyanText" placeholder="留言内容"></textarea>
			</view>
			<view class="btn">
				<button
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.xiaoyuanshiwuzhaolingId) {
				// 如果上一级页面传递了xiaoyuanshiwuzhaolingId，获取改xiaoyuanshiwuzhaolingId数据信息
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`xiaoyuanshiwuzhaolingLiuyan`, this.ruleForm.id);
		...
		methods: {
			styleChange() {
				this.$nextTick(() => {
					...
					//   el.style.backgroundColor = this.addUpdateForm.input.xiaoyuanshiwuzhaolingLiuyanText.backgroundColor
					// })
				})
			},
			...
			async onSubmitTap() {
				//跨表计算判断
				if ((!this.ruleForm.xiaoyuanshiwuzhaolingLiuyanText)) {
					this.$utils.msg(`留言内容不能为空`);
					return
				}
				await this.$api.add(`xiaoyuanshiwuzhaolingLiuyan`, this.ruleForm);
				uni.setStorageSync('xiaoyuanshiwuzhaolingLiuyanState', true);
