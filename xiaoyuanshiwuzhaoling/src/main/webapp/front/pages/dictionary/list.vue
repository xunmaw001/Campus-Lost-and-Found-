	...
	<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
		<view @tap="onDetailTap(product)" class="uni-product"
			...
			<view class="uni-product-title"
				...
				{{product.dictionaryName}}</view>
			<view class="image-view">
				<image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
					class="uni-product-image" :src="product.dictionaryPhoto?product.dictionaryPhoto.split(',')[0]:''"></image>
			</view>
			<view class="uni-product-price" v-if="product.dictionaryNewMoney != null"
				:style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
				<text class="">￥{{product.dictionaryNewMoney}}</text>
			</view>
			<view style="display: flex;justify-content: space-between;">
				<text v-if="isAuth('dictionary','修改')" class="cuIcon-edit"
					@click.stop="onUpdateTap(product.id)">修改</text>
				<text v-if="isAuth('dictionary','删除')" class="cuIcon-delete"
