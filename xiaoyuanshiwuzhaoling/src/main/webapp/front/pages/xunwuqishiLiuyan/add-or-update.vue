		...
		<form class="app-update-pv">
			<view
				...
				<textarea
					...
					v-model="ruleForm.xunwuqishiLiuyanText" placeholder="留言内容"></textarea>
			</view>
			<view class="btn">
				<button
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.xunwuqishiId) {
				// 如果上一级页面传递了xunwuqishiId，获取改xunwuqishiId数据信息
				this.ruleForm.xunwuqishiId = options.xunwuqishiId;
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`xunwuqishiLiuyan`, this.ruleForm.id);
		...
		methods: {
			styleChange() {
				this.$nextTick(() => {
					...
					//   el.style.backgroundColor = this.addUpdateForm.input.xunwuqishiLiuyanText.backgroundColor
					// })
				})
			},
			...
			async onSubmitTap() {
				//跨表计算判断
				if ((!this.ruleForm.xunwuqishiLiuyanText)) {
					this.$utils.msg(`留言内容不能为空`);
					return
				}
				await this.$api.add(`xunwuqishiLiuyan`, this.ruleForm);
				uni.setStorageSync('xunwuqishiLiuyanState', true);
