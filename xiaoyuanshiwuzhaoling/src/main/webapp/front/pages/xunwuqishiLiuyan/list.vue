	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.xunwuqishiLiuyanText" type="text" placeholder="评论内容"></input>
			</view>
			<view class="action">
				<button @tap="search"
		...
		<view class="list">
			...
			<view @tap="onDeleteTap(item.id)" v-for="(item,index) in list " v-bind:key="index" class="listm flex flex-between">
					<view class="listmr">
						<view class="colb f24 lh35">评论内容：{{item.xunwuqishiLiuyanTlext}}</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if (this.searchForm.xunwuqishiLiuyanText) {
					params['xunwuqishiLiuyanText'] = '%' + this.searchForm.xunwuqishiLiuyanText + '%'
				}

				let res = await this.$api.page(`xunwuqishiLiuyan`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xunwuqishiLiuyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.xunwuqishiLiuyanText) {
					searchForm['xunwuqishiLiuyanText'] = '%' + this.searchForm.xunwuqishiLiuyanText + '%'
				}
			
				let res = await this.$api.page(`xunwuqishiLiuyan`, searchForm);
				// 如果是第一页数据置空
