...
<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
		...
		<view class="search-form round">
			...
			<input v-model="searchForm.xiaoyuanshiwuzhaolingName" type="text" placeholder="失物招领名称"></input>
		</view>
		<view class="action">
			<button @tap="search"
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			v-for="(item,index) in shiwuzhaolingTypesList" :key="index"
	...
	<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
		<view @tap="onDetailTap(product)" class="uni-product"
			...
			<view class="uni-product-title"
				...
				{{product.xiaoyuanshiwuzhaolingName}}</view>
			<view class="image-view">
				<image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
					class="uni-product-image" :src="product.xiaoyuanshiwuzhaolingPhoto?product.xiaoyuanshiwuzhaolingPhoto.split(',')[0]:''"></image>
			</view>
			<view class="uni-product-price" v-if="product.xiaoyuanshiwuzhaolingNewMoney != null"
				:style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
				<text class="">￥{{product.xiaoyuanshiwuzhaolingNewMoney}}</text>
			</view>
			<view style="display: flex;justify-content: space-between;">
				<text v-if="isAuth('xiaoyuanshiwuzhaoling','修改') && user.id == product.yonghuId" class="cuIcon-edit"
					@click.stop="onUpdateTap(product.id)">修改</text>
				<text v-if="isAuth('xiaoyuanshiwuzhaoling','删除') && user.id == product.yonghuId" class="cuIcon-delete"
	...
	<button
            ...
            v-if="isAuth('xiaoyuanshiwuzhaoling','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.xiaoyuanshiwuzhaolingName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if (this.categoryName != '0') {
					params.shiwuzhaolingTypes = this.categoryName
				}
				let res = await this.$api.list(`xiaoyuanshiwuzhaoling`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xiaoyuanshiwuzhaoling', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(types) {
				...
				if (this.searchForm.xiaoyuanshiwuzhaolingName) {
					searchForm['xiaoyuanshiwuzhaolingName'] = this.searchForm.xiaoyuanshiwuzhaolingName
				}
				
				let res = await this.$api.list(`xiaoyuanshiwuzhaoling`, searchForm);
				// 如果是第一页数据置空
