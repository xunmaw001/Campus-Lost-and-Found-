		...
		<view class="container">
			...
			<view class="detail-content">
				...
				<view class="name shop-title"
					...
					物品名称：{{detail.xiaoyuanshiwuzhaolingName}}
				</view>
				...
				<view class="detail-list-item"
					...
					<view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
						{{detail.xiaoyuanshiwuzhaolingUuidNumber}}</view>
				</view>
				<view class="detail-list-item"
					:style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
					<view class="lable"
				...
				<view class="detail-list-item"
					...
					<view class="lable"
						...
						拾遗时间：</view>
					<view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
						{{detail.xiaoyuanshiwuzhaolingTime}}</view>
				</view>
				<view class="detail-list-item"
					:style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
					<view class="lable"
						...
						拾遗地址：</view>
					<view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
						{{detail.xiaoyuanshiwuzhaolingDizhi}}</view>
				</view>
				<view class="detail-list-item"
					:style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
					<view class="lable"
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.xiaoyuanshiwuzhaolingContent"></rich-text>
				</view>
			</view>
					...
					<view
						...
						<view class="content">
							...
							<view :style='{"color":"#666","textAlign":"left","fontSize":"24rpx"}'
								...
								{{item.xiaoyuanshiwuzhaolingLiuyanText}}
							</view>
						</view>
					</view>
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				...
				<view style="text-align: right;">
					<button @tap="onUpdateXinxiTypes(detail)" style="margin-right: 10upx;min-width: 180upx;"
						...
						:style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[2],borderColor:btnColor[2]}">申请领取</button>
				</view>
			</view>
	...
	export default {
		data() {
			return {
				...
				hasNext: true,
				user: {},
				xiaoyuanshiwuzhaolingCollectionFlag: 0,
				count: 0,
				timer: null
			}
		},
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('xiaoyuanshiwuzhaolingLiuyanState')
			if (cleanType) {
				uni.removeStorageSync('xiaoyuanshiwuzhaolingLiuyanState')
				this.mescroll.num = 1
		...
		methods: {
			// 获取详情
			async init() {
				let res = await this.$api.info('xiaoyuanshiwuzhaoling', this.id);
				this.detail = res.data;
				// 轮播图片
				this.swiperList = this.detail.xiaoyuanshiwuzhaolingPhoto ? this.detail.xiaoyuanshiwuzhaolingPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('xiaoyuanshiwuzhaolingLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					xiaoyuanshiwuzhaolingId: this.id
				});
				// 如果是第一页数据置空
			...
			onUpdateXinxiTypes(detail){
				detail.xinxiTypes = 2
				this.$api.update('xiaoyuanshiwuzhaoling', detail);
				uni.redirectTo({
				    url: '/pages/shiwurenling/add-or-update?shiwurenlingId='+detail.id
				});
			},
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../xiaoyuanshiwuzhaolingLiuyan/add-or-update?xiaoyuanshiwuzhaolingId=${this.id}`)
			},
		}
	}
</script>
